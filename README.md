...
중단은 `"enabled": false` 또는 `expires_at`을 과거로 두면 된다. 지난 회차는 자동으로 제외된다 — 지난 날짜뿐 아니라 오늘 이미 시작된 시각도 빠지므로, 대상이 전부 지나간 모니터는 그것만으로 비활성이 된다. 유예 시간은 없다(시작 시각과 같으면 이미 지난 것으로 다룬다).
...
**`감시 실패` 알림이 왔다**
연속 5회(약 6~8분) 동안 결과를 하나도 얻지 못했다는 뜻이다. 모든 회차 조회가 실패한 경우와 예상 못한 내부 오류로 회차가 통째로 날아간 경우를 함께 센다. 알림을 시도한 뒤 Naver 요청 Session을 한 번 초기화하고, job·조회 주기·연결 실행은 그대로 유지한 채 다음 회차에서 재시도한다.

Actions 로그의 `error=` 값을 본다. `error=` 값이 있으면 `malformed_response`(파서 수정 필요)인지 `rate_limited`(간격 상향)인지 판단한다. `error=`가 아예 없고 `예상 못한 오류` 스택 트레이스만 있으면 코드 버그다.

전송에 성공할 때까지 회차마다 다시 시도하지만, 한 번 성공하면 해당 장애가 해소될 때까지 반복하지 않는다. 이후 하나라도 정상 조회하면 장애 상태를 먼저 해제하고 `감시 복구` 알림을 한 번 보낸다. 복구 알림 전송에 실패하면 다음 성공 회차에서 다시 시도하되, 그 사이 새 장애가 나면 새 `감시 실패` 알림도 보낸다. **job은 실패시키지 않는다** — 연결 실행을 끊고 지연이 큰 cron에 복구를 맡기지 않기 위해서다.

**`상태 저장 실패` 알림이 왔다**
상태 파일을 원격에 남기지 못했다. 다음 실행이 예전 상태로 시작하므로 같은 알림이 다시 올 수 있다. 감시 자체는 계속된다.

본문이 두 가지를 구분한다.

- `rebase 실패(충돌 또는 인증)` — `git pull --rebase`가 실패했다. 상태 파일 충돌일 수도 있고, checkout이 저장한 인증 정보가 깨진 것일 수도 있다. Actions 로그의 `Commit state` 단계에서 실제 git 오류를 확인한다.
- `푸시 2회 실패` — rebase는 됐지만 push가 두 번 다 밀렸다. 보통 다른 실행과의 경합이다.

어느 쪽이든 다음 실행이 원격의 최신 상태를 다시 읽으므로 수동 개입은 필요하지 않다.
