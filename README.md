...
**`감시 중단` 알림이 왔다**
연속 5회(약 6~8분) 동안 결과를 하나도 얻지 못했다는 뜻이다. 모든 회차 조회가 실패한 경우와 예상 못한 내부 오류로 회차가 통째로 날아간 경우를 함께 센다. 감시가 사실상 멈춰 있다.

본문의 `점검: Actions 로그의 error= 값`대로 로그를 본다. `error=` 값이 있으면 `malformed_response`(파서 수정 필요)인지 `rate_limited`(간격 상향)인지 판단한다. `error=`가 아예 없고 `예상 못한 오류` 스택 트레이스만 있으면 코드 버그다.

전송에 성공할 때까지 회차마다 다시 시도하지만, 한 번 성공하면 정상 조회가 나오기 전까지 반복하지 않는다. **job은 실패시키지 않는다** — 연결 실행을 끊고 지연이 큰 cron에 복구를 맡기지 않기 위해서다.
