...
전송에 성공할 때까지 회차마다 다시 시도하지만, 한 번 성공하면 복구 전까지 반복하지 않는다. 이후 하나라도 정상 조회하면 `감시 복구` 알림을 한 번 보낸다. 복구 알림 전송에 실패하면 다음 성공 회차에서 다시 시도한다. **job은 실패시키지 않는다** — 연결 실행을 끊고 지연이 큰 cron에 복구를 맡기지 않기 위해서다.
